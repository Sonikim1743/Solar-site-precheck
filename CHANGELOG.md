...
- 最大直列数の入力時に、低温時Voc上昇とPCS最大入力電圧の確認が必要であることをヘルプで表示。
- PCS・モジュール結線アシストで、PCS最大入力電圧から求めた直列上限を計算候補に反映し、電圧上限目安を画面に表示。
- Cloudflare / Portable の実行環境判定を整理し、PDF処理目安容量が実行環境に合わせて表示されるように改善。
- 更新スクリプトにZIPのSHA-256照合と古いバックアップ整理を追加し、OpenClaw経由の更新安全性を改善。
- Cloudflare配布用ヘッダーから不要になったService Worker指定を整理。
- 海岸部など一部サンプル地点で国土地理院DEM標高が取得できない場合でも、取得できた地点だけで地平線分析を継続できるように改善。
- トップ画面の「入力マニュアル」から、社内共有用の操作案内PDFを直接保存できるように変更。
- 操作案内PDF `site-operation-guide-v1.23.pdf` をCloudflare版で参照できるようにし、軽量更新ZIPからは除外して更新容量を抑制。
