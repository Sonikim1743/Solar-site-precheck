...
export default function PowerGridPage({position,placeLabel,powerGrid,gridCapacity,onCheck,onLoadCapacity,onBack,onReport,annualYield='',generation=null,onGenerationChange,generationInputs,onGenerationInputsChange,candidateRevision=0,onGenerationNotice}){
 ...
 <p className="power-page__hint">別画面へ移動すると設備の選択は解除されます。必要な内容はメモに保存してください。</p><button className="power-page__primary" onClick={saveNote}>設備確認メモを保存</button>{noteStatus&&<p role="status">{noteStatus}</p>}</section>}
 ...
 <GenerationPanel key={candidateRevision+":"+siteKey} onNotice={onGenerationNotice} position={position} result={generation} onChange={onGenerationChange} annualYield={annualYield} draftInputs={generationInputs} onInputsChange={onGenerationInputsChange}/><p className="power-page__footnote">地図：OpenStreetMap・国土地理院。未登録の設備は表示されません。空容量は公表時点の参考情報で、接続可否・工事費・工期は電力会社への確認が必要です。</p>
 </section>
}
