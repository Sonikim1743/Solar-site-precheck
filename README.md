...
- Solar Proへの直接入力は行いません。現時点ではメモ、CSV、画面確認、手入力支援が基本です。
...
- 候補地データのJSONエクスポート・インポート
- 公式仕様が確認できた場合のSolar Pro地平線ファイル出力
