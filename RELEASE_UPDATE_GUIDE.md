# v1.27.2 更新予定 — 2026-09-24

**v1.27.2は作業・確認予定です。この文書は実装・公開・PC更新の完了を示しません。** 現在の配布済み版はv1.27.1で、実際の公開・PC更新の証拠は `docs/DEPLOYED_VERSION.md` を確認してください。

今回の明示的な変更は、断面ボタンを余分な開閉なしで常時表示し、地点・標高・コピーの間隔を整えることです。**発電量だけを初期メインから外し、上部メニューから開く別画面にします。** 地平線・積雪・筆の検討・レポート・資料・SolarProマニュアルは、メインの折りたたみ構成を維持します。v1.27.1の同じページという方針に対する発電量だけの例外であり、他の機能まで分離する変更ではありません。GEONEXの削除も維持します。

配布前に変更した操作と、発電量画面の往復で地点・入力・比較条件・結果が保持されることを確認します。未変更の計算・記録検証・図形処理には既存の自動テストの根拠を使い、実際に確認した画面範囲と限界を記録します。予定範囲は `docs/RELEASE_v1.27.2_KO.md` を参照してください。

更新前に必要な検討記録を保存し、サーバーを止めて実行中フォルダーをバックアップします。既存runtime/node.exeと個人データを維持してください。対象別手順は `docs/DEPLOYMENT_PACKAGE.md` を使い、実際の配布物・バージョン・チェックサムを更新メタデータと照合します。旧schema 1の記録は読み込めますが、新schema 2の記録は旧版アプリでは開けません。コード開発はMacを中心にし、`docs/MAC_OPENCLAW_START_KO.md` に従います。

## v1.27.1 配布済みの履歴 — 2026-09-24

GitHub、既存Cloudflare Pages、Windowsローカル実行版を更新しました。配布物は `SolarSitePrecheck_v1.27.1_release_light.zip` です。当時は発電量も含め同じページの折りたたみ構成を復元しましたが、次のv1.27.2では発電量だけを別画面にする明示変更を適用します。配布証拠は `docs/DEPLOYED_VERSION.md`、当時の画面確認は `docs/V1_27_1_BROWSER_CHECK_KO.md`、変更履歴は `docs/RELEASE_v1.27.1_KO.md` にあります。
